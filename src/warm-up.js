...
// comparison operator.
var isShortStory = checkShortStory(pages);

function checkShortStory(pages) {
  if(pages < 25) {
    return true;
  } else {
    return false;
...
function determineReadSpeed(pages){
  if (pages < 1000){
...
console.log(determineReadSpeed(pages));
pages = 5000;
console.log(determineReadSpeed(pages));
...
function checkBook(title, isFavorite){
  if (isFavorite === true) {
...
console.log(checkBook(title, isFavorite));
isFavorite = false;
console.log(checkBook(title, isFavorite));

//------------------------- Arrays -------------------------//

var books = ["Green Eggs and Ham", "Harry Potter", "Dracula", "Dune", "Of Mice and Men"];
console.log(books);
...
var belowTwoHundred = [101, 99, 89, 43, 54, 65, 23, 190, 70, 33, 23, 42];
